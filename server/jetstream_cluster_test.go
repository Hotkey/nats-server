...
func TestJetStreamClusterMirrors(t *testing.T) {
	...
	createStream(&StreamConfig{Name: "M1", Storage: FileStorage, Mirror: "S1", Placement: &Placement{Cluster: "C1"}})
	...
	createStream(&StreamConfig{Name: "M2", Storage: FileStorage, Mirror: "S1", Replicas: 3, Placement: &Placement{Cluster: "C3"}})
...
func TestJetStreamClusterSources(t *testing.T) {
	...
	for _, sname := range []string{"foo", "bar", "baz"} {
		if _, err := js.AddStream(&nats.StreamConfig{Name: sname, Replicas: 2}); err != nil {
			t.Fatalf("Unexpected error: %v", err)
		}
	}
	...
	if sl == sc.leader() {
		fmt.Printf("META LEADER SAME AS SHUTDOWN!!\n\n")
	}
